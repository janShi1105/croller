...
app = Celery('crawler_celery', broker='redis://localhost:6379/0')
app.conf.update(
    task_serialize='json',
    accept_content=['json'],
    ...
    task_routes={
        'crawler_celery.download':{
            'queue': 'download',
            'routing_key': 'download'
        },
        'crawler_celery.cut_mp3':{
            'queue': 'media',
            'routing_key': 'media'
        }
    }
...
def cut_mp3(file_name):
    ...
    root_name, ext = path.selitext(file_name)
    ...
    file_handler.close()
    logger.info("[cut_mo3 finished] {file_name}".format(file_name=file_name))
